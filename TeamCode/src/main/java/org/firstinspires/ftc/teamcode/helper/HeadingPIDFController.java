...
public class HeadingPIDFController {
    ...
    public void setTargetPosition(double targetPosition) {
        while (targetPosition < 0) {
            targetPosition += 2 * Math.PI;
        }
        this.targetPosition = targetPosition % (2 * Math.PI);
        sumError = 0;
        time.reset();
    }
    ...
    public double angleWrapper(double radians) {
    ...
    public double update(double processVariable) {
        double error = angleWrapper(targetPosition - processVariable);
        double output = kf;
